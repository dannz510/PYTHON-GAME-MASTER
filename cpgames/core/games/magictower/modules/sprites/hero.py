...
class Hero(pygame.sprite.Sprite):
    ...
    def dealcollideevent(self, elem, block_position, map_parser, screen):
        ...
        # 捡到不同颜色的钥匙
            ...
            if elem == '6':
                self.num_yellow_keys += 1
                self.obtain_tips = 'Get a yellow key'
            elif elem == '7':
                self.num_purple_keys += 1
                self.obtain_tips = 'Get a blue key'
            elif elem == '8':
                self.num_red_keys += 1
                self.obtain_tips = 'Get a red key'
            return True, []
        # 捡到宝石
        elif elem in ['9', '10']:
            if elem == '9':
                self.defense_power += 3
                self.obtain_tips = 'Get a blue gem, defense + 3'
            elif elem == '10':
                self.attack_power += 3
                self.obtain_tips = 'Get a ruby, attack power + 3'
            return True, []
        # 捡到血瓶
        elif elem in ['11', '12']:
            if elem == '11':
                self.life_value += 200
                self.obtain_tips = 'Get a small blood bottle, life + 200'
            elif elem == '12':
                self.life_value += 500
                self.obtain_tips = 'Get a large blood bottle, add health + 500'
            return True, []
        # 上下楼梯
        elif elem in ['13', '14']:
        ...
        # 捡到道具飞羽
            ...
            if elem == '30':
                ...
                self.obtain_tips = 'Get a small flying feather and increase your level by one level'
            elif elem == '31':
                self.level += 3
                self.life_value += 3000
                self.attack_power += 21
                self.defense_power += 21
                self.obtain_tips = 'Get the Big Feather and level up by 3 levels'
            return True, []
        # 捡到幸运十字架
        elif elem in ['32']:
            self.has_cross = True
            self.obtain_tips = ['【Lucky Cross】 Give it to the fairy in the prologue', 'You can improve all your abilities (attack, defense and health)']
            return True, []
        # 捡到圣水瓶
        elif elem in ['33']:
            self.life_value *= 2
            self.obtain_tips = '【Holy Water Bottle】 It can double your physical strength'
            return True, []
        # 捡到圣光徽
        elif elem in ['34']:
            self.has_forecast = True
            self.obtain_tips = '【Holy Light Badge】 Press L to view the monster basic information'
            return True, []
        # 捡到风之罗盘
        elif elem in ['35']:
            self.has_jump = True
            self.obtain_tips = '【Wind Compass】 Press J to jump between floors you have walked through.'
            return True, []
        # 捡到钥匙盒
        elif elem in ['36']:
            self.num_yellow_keys += 1
            self.num_purple_keys += 1
            self.num_red_keys += 1
            self.obtain_tips = 'Get the key box. Add 1 to the number of keys.'
            return True, []
        # 捡到星光神榔
        elif elem in ['38']:
            self.has_hammer = True
            self.obtain_tips = ['【Starlight Divine Palm】 Give it to the thief on the fourth floor', 'The thief will use it to open the hidden floor of the eighteenth floor']
            return True, []
        # 捡到金块
        elif elem in ['39']:
            self.num_coins += 300
            self.obtain_tips = 'Get gold nuggets. Gold coin + 300'
            return True, []
        # 遇到怪物
            ...
            if self.winmonster(monster)[0]:
                ...
                self.obtain_tips = f'Number of coins obtained{monster[4]} Experience{monster[5]}'
                return True, []
            else:
                return False, []
        # 得到铁剑
            ...
            self.attack_power += 10
            self.obtain_tips = 'Get Iron Sword Attack Power +10'
            return True, []
        # 得到钢剑
        elif elem in ['73']:
            self.attack_power += 30
            self.obtain_tips = 'Get a steel sword, Attack Power + 30'
            return True, []
        # 得到圣光剑
        elif elem in ['75']:
            self.attack_power += 120
            self.obtain_tips = 'Get Holy Light Sword, Attack Power + 120'
            return True, []
        # 得到铁盾
        elif elem in ['76']:
            self.defense_power += 10
            self.obtain_tips = 'Get Iron Shield Defense + 10'
            return True, []
        # 得到钢盾
        elif elem in ['78']:
            self.defense_power += 30
            self.obtain_tips = 'Get Steel Shield Defense +30'
            return True, []
        # 得到星光盾
        elif elem in ['80']:
            self.defense_power += 120
            self.obtain_tips = 'Get Starlight Shield, Defense + 120'
            return True, []
        # 其他
        else:
            return False, []
